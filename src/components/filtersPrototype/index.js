...
export default function FilterPrototype({ search, select }) {
    return (
        <Box sx={{
            height: "60px", padding: "0 15px", width: "95%",
            ...
            {search && (
                <TextField onChange={({ target }) => search.onChange(target.value)} sx={{ width: "150px" }} size="small" id="outlined-basic" label={search.label} variant="outlined" />
            )}
