...
export default function ListPrototype({
  header,
  list,
  clickable,
  recharge,
          ...
          <div className={Style.contentListOt}>
            {list && list[0] ? (
              list.slice(count * 20, count * 20 + 20).map((data, key) => (
                <div
                  onMouseEnter={() => setDivHover(data.id)}
                  onMouseLeave={() => setDivHover(null)}
                  key={key}
                  className={Style.ColumOt}
                  onDoubleClick={() => clickable && clickable(data)}
                >
                  <Table
                    recharge={recharge}
                    data={data}
...
const ComponentSkeleton = () => {
  const [skeletons, setSkeletons] = useState([
    <Skeleton width={"90%"} height={"100%"} key={1} />,
    <Skeleton width={"90%"} height={"100%"} key={2} />,
    <Skeleton width={"90%"} height={"100%"} key={3} />,
    <Skeleton width={"90%"} height={"100%"} key={4} />,
    <Skeleton width={"90%"} height={"100%"} key={5} />,
    <Skeleton width={"90%"} height={"100%"} key={6} />,
    <Skeleton width={"90%"} height={"100%"} key={7} />,
  ]);
  useEffect(() => {
