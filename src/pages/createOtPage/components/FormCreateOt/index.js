...
function FormCreateOt() {
  ...
  const { OT, allActivities, editOT, getOt, verifyOT, resetOt } = useCreateOT();
  const { data: allTypes } = useFetchUrl("/getTypeOt");
  const { data: clientsData } = useFetchUrl("/getClients");
  const clientsFormate = clientsData
  ...
  const handleChangeDescription = (value, number, type) => {
    ...
    copy[number][type] =
      type === "Description" && value ? toUppercase(value) : value;
    editOT("Description", copy);
  };
  const handleType = (value) => {
    editOT("Activities", JSON.parse(allTypes[value].activities));
      ...
      <div className={Style.idOT}>
        ...
        <BootstrapInput
          value={
            OT.OTKey +
            " " +
            isNullUndefined(OT.Type?.abbreviation) +
            " " +
            isNullUndefined(OT.Client?.KeyUnique)
          }
          disabled
          id="ot-id"
          variant="outlined"
        ...
        <Box sx={{ paddingTop: "10px" }}>
          <LocalizationProvider dateAdapter={AdapterDayjs}>
            <DatePicker
              ...
              value={OT.Date}
              onChange={(value) => editOT("Date", value)}
            />
        ...
        {allActivities && (
          <Select
            ...
            onChange={({ target: { value } }) =>
              editOT("Activities", { valueSelected: value, allActivities })
            }
            value={OT.Activities}
            multiple
            placeholder="Selecciona el tipo de OT"
        ...
        {OT.Client && OT.Client.Contacts[0] && (
          <MultipleSelect
            ...
            onchange={(value) =>
              editOT(
                "Contact",
                value.map((data) => Number(data[0]) - 1)
              )
            }
            names={OT.Client.Contacts.map(
              (ContactClient, key) =>
                `${key + 1} ${ContactClient.type}: ${ContactClient.contact} ${
                  ContactClient.email
                }`
            )}
        ...
        <div className={Style.inputsContainer}>
          <div className={Style.ClientData}>
            <div className={Style.ClientDataContent}>
              ...
              <BootstrapInput
                style={{ width: "100%" }}
                value={isNullUndefined(OT.Client?.idEditable)}
                disabled
                id="client-number"
                variant="outlined"
              />
            ...
            <div className={Style.ClientDataContent}>
              ...
              <BootstrapInput
                style={{ width: "100%" }}
                value={isNullUndefined(OT.Client?.KeyUnique)}
                disabled
                id="client-key"
                variant="outlined"
              />
          ...
          <div className={Style.DataInput}>
            <CustomInput
              value={OT.Modelo}
              onChange={(e) => editOT("Modelo", e)}
              placeholder={"Modelo"}
            />
          </div>
          <div className={Style.DataInput}>
            <CustomInput
              value={OT.NormaAplicar}
              onChange={(e) => editOT("NormaAplicar", e)}
              placeholder={"Norma a aplicar"}
            />
          </div>
          <div className={Style.DataInput}>
            <CustomInput
              value={OT.Cotizacion}
              onChange={(e) => editOT("Cotizacion", e)}
          ...
          <div className={Style.DataInput}>
            <CustomInput
              value={OT.nLacre}
              onChange={(e) => editOT("nLacre", e)}
              placeholder={"Numero de Lacre"}
            />
          </div>
          <div className={Style.DataInput}>
            <TextField
        ...
        <div className={Style.inputsContainer}>
          {OT.Description.map((data, key) => (
            <div key={key} className={Style.DataInput}>
              ...
              <TextField
                ...
                onChange={(e) =>
                  handleChangeDescription(e.target.value, key, "Description")
                }
                id="outlined-multiline-flexible"
                multiline
                maxRows={3}
          ...
          <Button
            onClick={() =>
              editOT("Description", [
                ...OT.Description,
                { item: "", Description: "", import: 0 },
              ])
            }
            variant="outlined"
            sx={{ borderRadius: "15px", margin: "15px 0", width: "100%" }}
          >
            <div>
              <AddIcon />
            </div>
          ...
          <div className={Style.dateSection}>
            ...
            <LocalizationProvider dateAdapter={AdapterDayjs}>
              <DatePicker
                ...
                format="DD/MM/YYYY"
                slotProps={{ textField: { size: "small" } }}
                value={OT.FechaEstimada}
                onChange={(e) => editOT("FechaEstimada", e)}
              />
            </LocalizationProvider>
          </div>
          <div className={Style.dateSection}>
            <p className={Style.TittleType}>
            ...
            <LocalizationProvider dateAdapter={AdapterDayjs}>
              <DatePicker
                ...
                format="DD/MM/YYYY"
                slotProps={{ textField: { size: "small" } }}
                value={OT.FechaVencimiento}
                onChange={(e) => editOT("FechaVencimiento", e)}
              />
            </LocalizationProvider>
          </div>
        </div>
